...
def train(model, train_data_loader, val_data_loader, experiment_name, log_wandb, output_dir, project_name=None, use_ray_tune=False):
    ...
    if torch.cuda.is_available():
        model.cuda()
    # weights = torch.tensor([0.1, 2000.0, 1.0]).to(device)
    weights = torch.tensor([0.1, 2000, 1.0]).to(device)
    ...
    for i in range(model.epochs):
        ...
        for features, labels, time_feature, times, norm_inputs in train_data_loader:
            ...
            if model.is_rnn:
                h = h.data
            if model.is_rnn:
                output, o, _, h, acc_output,_ ,_= model(features, norm_inputs, labels,  h, time_feature, times)
            else:
                output, o, _, _, acc_output,_ ,_ = model(features, norm_inputs, labels, None, time_feature, times)

            # print("output shape :",output.shape)
            # print("labels shape :",labels.shape)
            
                
            loss = model.weighted_mse_loss(o, labels, weights).mean()
        ...
        for val_f, val_la, val_timef, val_times, val_normin in val_data_loader:
            ...
            if model.is_rnn:
                ...
                out, o, _, val_h, _ ,_,_ = model(val_f, val_normin, val_la, val_h,val_timef, val_times)
            else:
                out, o, _, _, _,_,_  = model(val_f, val_normin, val_la, None, val_timef, val_times)

            val_loss = model.weighted_mse_loss(o, val_la, weights)
        ...
        if mean_val_loss < valid_loss_min:
            print('Validation loss decreased ({:.9f} --> {:.9f}).  Saving model ...'.format(valid_loss_min,mean_val_loss))
        ...
        print("Epoch: {}/{}...".format(i+1, model.epochs),
            "Loss: {:.15f}...".format(mean_train_loss),
            "Val Loss: {:.15f}".format(mean_val_loss),
            "loss1: {:.15f}, loss2: {:15f}, alpha1: {:.15f}, alpha2: {:.15f}".format(loss_items[0].item(),loss_items[1].item(),alpha_item[0].item(),alpha_item[1].item()))
        ...
        if i % 100 ==0:
            ...
            print("rmse vx", rmse_val_vx)
            print("rmse vy", rmse_val_vy)
            print("rmse yawrate", rmse_val_yawr)
        if use_ray_tune:
            checkpoint_data = {
                "epoch": i,
    ...
    model.load_state_dict(torch.load("%s/best_before_model.pth" % (output_dir),weights_only=True))
    for param in model.parameters():
        param.requires_grad = True
        
    freeze_layers_num = int(len(model.feed_forward)*1/2)
    ...
    for i in range(additional_epoches):
        ...
        for features, labels, time_feature, times, norm_inputs in train_data_loader:
            ...
            times = times.requires_grad_(True)
            time_feature = time_feature.requires_grad_(True)
            # print(time_feature)sss
            if model.is_rnn:
                h = h.data
            if model.is_rnn:
                output, o, n, h, acc_output,_ ,_ = model(features, norm_inputs, labels, h, time_feature, times)
            else:
                output, o, n,  _, acc_output,_ ,_ = model(features, norm_inputs, labels, None, time_feature, times)
            ...
            loss1 = loss1.mean()
            loss2 = (df_error(labels, acc_output,times)**2).mean()
            times.detach()
        ...
        for val_f, val_la, val_timef, val_times, val_normin in val_data_loader:
            ...
            if model.is_rnn:
                val_h = val_h.data
                out, o, n, val_h, _ ,_ ,_= model(val_f, val_normin, val_la, val_h,val_timef, val_times)
            else:
                out, o, n, _, _ ,_ ,_ = model(val_f, val_normin, val_la, None, val_timef, val_times)
            ...
            val_la = val_la - n
            label_ls.append([val_la[:-1,:],val_f[1:,-1,3:],val_times[:-1,:]])
            # time_ls.append(val_times)
            val_vx_accum += val_loss[:,0].mean().item()/weights[0].item()
            val_vy_accum += val_loss[:,1].mean().item()/weights[1].item()
            val_yawr_accum += val_loss[:,2].mean().item()/weights[2].item()
        ...
        if mean_val_loss < valid_loss_min:
            print('Fine Validation loss decreased ({:.9f} --> {:.9f}).  Saving model ...'.format(valid_loss_min,mean_val_loss))
            torch.save(model.state_dict(), "%s/finetuned_model.pth" % (output_dir))
            valid_loss_min = mean_val_loss
            best_coeff = model.get_trained_coeff()
            print("best val loss:",mean_val_loss)
            
            arrays = [[tensor.detach().cpu().numpy() for tensor in inner_list] for inner_list in label_ls]
            # time_arrays = [[tensor.detach().cpu().numpy() for tensor in inner_list] for inner_list in time_ls]
            
            concatenated_arrays = [np.concatenate(inner_list, axis=1) for inner_list in arrays]
            # concatenated_time_arrays = [np.concatenate(inner_list, axis=0) for inner_list in time_arrays]
            
            final_array = np.vstack(concatenated_arrays)
            # time_final = np.vstack(concatenated_time_arrays)
            # print(final_array.shape)
            arrays_dict = {'features':np.array(final_array)}
            np.savez('%s/denoised_csv.npz' % (output_dir), **arrays_dict)
            print("saved to denoised data")
            if log_wandb:
                wandb.log({"best_val_loss" : mean_val_loss})
                coeff = []
                for i in range(len(parameters_ls)):
    ...
    print("val loss : {:.9f}".format(valid_loss_min))
